...
// the model ID
const ID = 'http://privacy.pn.schema.webshield.io';

//------------------------
//-------------------------
// MOST OF CODE BELOW THIS POINT IS DEPRECATED NEED TO LOOK AT HOW REMOVE
//---------------------------
//---------------------------

//
// DEPRECATED CODE - see code in metadata package
//
let URLS = {
  P:      ID + '/prop#',
  OCTX:   ID + '/octx#',
...
//
// DEPRECATED CODE as v1 privacy pipe - see code in connector-utils
//
model.utils.createPrivacyPipeResource = function createPrivacyPipeResource(props) {
  ...
  if ((props.destinationProvisionPipeURL) && (props.destinationProvisionPipeURL['@type'])) {
    ppReq[PN_P.destinationProvisionPipeURL] = props.destinationProvisionPipeURL;
  } else {
    ppReq[PN_P.destinationProvisionPipeURL] = PNDataModel.utils.createURLValue(props.destinationProvisionPipeURL);
  }
...
//
//
// DEPRECATED CODE - see encrypt key metadata in metadata package
//
...
//
//
// DEPRECATED CODE - now tag obfuscated values with PAI @id
//
...
model.utils.createPrivacyAlgorithm = function createPrivacyAlgorithm(props) {
  ...
  var pa = {};
  pa['@type'] = [PN_T.Metadata, PN_T.PrivacyAlgorithm, PN_T.Resource];
