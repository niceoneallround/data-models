...
// *pn_p.privacy_action - array of pn_t.PrivacyAction that obfuscate or de-obfuscate data, ok for there to be none in which
...
// pipe provides the client and next.
//
model.utils.createNonInstantiatedPrivacyStep = function createNonInstantiatedPrivacyStep(props) {
  assert(props, 'props param missing');
  assert(props.id, util.format('props.id is missing:%j', props));
  assert(!props.client, util.format('props.client should not be passed in:%j', props));
  assert(!props.next, util.format('props.next should not be passed in :%j', props));
