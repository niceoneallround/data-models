...
model.utils.createPrivacyPipeResource = function createPrivacyPipeResource(props) {
  ...
  if (props.action === PN_T.Obfuscate) {
    ...
    assert(!props.privacyAlgroithm,
      util.format('createPrivacyPipe: should not pass a privacy Algorithm for de-obfuscate as gets from pipe:%j', props));

    // must pass in the privacy pipe that was used to obfuscate the data as needed by the PB to setup
    // the de-obfuscate pipe
    assert(!props.privacyPipe, util.format('createPrivacyPipe: for de-obfuscate need the obfuscate privacy pipe:%j', props));
    ppReq[PN_P.privacyPipe] = props.privacyPipe;
  }
