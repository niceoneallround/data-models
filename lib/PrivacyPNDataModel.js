...
model.utils.createPrivacyAction = function createPrivacyAction(props) {
  ...
  assert(!props.obfuscation_service, 'add code for obfuscaiton_service');
  assert(props.privacy_schema, 'add code for privacy_schema');
  var pa = {};
  pa['@id'] = props.id;
  pa['@type'] = PN_T.PrivacyAction;

  // in future may make optional and just populate at runtime, but for now
  // require
  pa[PN_P.patag] = props.patag;

  // process privacy schema to make sure ok
  if (!Array.isArray(props.privacy_schema)) {
    pa[PN_P.privacySchema] = [props.privacy_schema];
  } else {
    pa[PN_P.privacySchema] = props.privacy_schema.slice();
  }
  ...
  pa[PN_P.privacySchema].forEach(function (item) {
    ...
    assert(item[PN_P.propType], util.format('SchemaItem:%j does not have property %s', item, PN_P.propType));
    ...
    if (Array.isArray(item[PN_P.propType])) {
      item[PN_P.propType] = item[PN_P.propType].slice();
    }
