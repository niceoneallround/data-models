...
TYPE = {
  ...
  //
  CreateCollection:     URLS.T + 'CreateCollection'

};

//
// Utility routines that routine Promises that are specific to this data model and jsonld context
//
model = {};
model.promises = {};
...
model.promises.findObjects = function findObjectsPromise(doc, type) {
  ...
  // get PNDataModel context
  getContextPromise = contextUtils.promises.getContext(ID);

  // return promise whose then will return the results
  return getContextPromise.then(
    function (context) {
      var findObjectPromise = jsonldUtils.promises.findObjects(
                                    doc,
                                    type,
      ...
      return findObjectPromise.then(
        ...
        function (err) {
          throw new Error(util.format('Failed in findObject: %s', err));
        }
      );
    },

    function (err) { // err on getContextPromise
      throw new Error(util.format('Failed in getContext: %s', err));
    }
  );
};
