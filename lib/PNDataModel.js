...
var PROPERTY, TYPE, URLS, ID, model = {}, metaModel, ID2, httpHeaders = {};
...
PROPERTY = {
  ...
  identitySyndicationAlgorithm:    URLS.P2 + 'identitySyndication_algorithm',
  ...
  postBackUrl:              URLS.P2 + 'postBackUrl',
...
TYPE = {
  ...
  SubjectQueryResponse:     URLS.T2 + 'SubjectQueryResponse', // response
  SubjectQueryRestriction:  URLS.T2 + 'SubjectQueryRestriction', // the node represents the restriction subject in the query
...
model.ENUMS = {
  SyndicationJobStatusEnum: {
    FinishedError: { code: 'FinishedError', friendly:'Syndication Finished with an error' },
    FinishedOK: { code: 'FinishedOK', friendly: 'Syndication was successful' },
    New: { code: 'New', friendly: 'New Syndication Job' },
    Mock: { code: 'Mock', friendly: 'Mock for testing does nothing' },
    Processing: { code: 'Processing', friendly: 'Processing' },
    StoppedError: { code: 'StoppedError', friendly: 'Stopped due to some error' },
    WaitingForRS: { code: 'WaitingForRS', friendly: 'Waiting for RS to return subject query results' }
  }
};
...
model.ids.createMessageId = function createMessageId(hostname, id) {
  var type = 'https://pn.id.webshield.io/message';
...
//
model.ids.paramUtils.createMdIdFromParam = function createMdIdFromParam(id) {
  assert(id, 'id param is missing');
  var partAfterHash, part2Replace;

  //console.log('id:%s', id);

  // extract the part between the md prefix and the double dash as want to replace any '-' with '/'
  part2Replace = id.substring(
                    0,
                    id.indexOf('--'));
  ...
  partAfterHash = id.substring(
                    id.indexOf('--') + 2,
                    id.length);

  //console.log('partAfterHash:%s', partAfterHash);
  return model.ids.getMdIdPrefix() + '/' + part2Replace.replace(/[-]/g, '/') + '#' + partAfterHash;
};
