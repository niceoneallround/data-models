...
PROPERTY = {
  ...
  encryptKeyMD:             URLS.P2 + 'encrypt_key_md', //privacy action instance
  encryptKeyMDJWT:          URLS.P2 + 'encrypt_key_md_jwt', // privacy action instance
  ...
  jsonContext:              URLS.P2 + 'json_context', // used in data model
  ...
  pnDataModel:              URLS.P2 + 'pndatamodel', // used in privacy agents
  port:                     'port',
...
TYPE = {
  ...
  // PN Resource Types
  //
  InjestPrivacyAgent:         URLS.t2 + 'InjestPrivacyAgent', // injest privacy agent
  ...
  PNDataModel:                URLS.TS + 'PNDataModel',
...
//
// Create a syndicaiton request id of a known format - that will be unique for log files
// *hostname - the hostname of called for example acme.com
// *id - an id to be added to end
//
...
//
// Create a encrypt_key metadata id of a known format - that will be unique for log files
// *hostname - the hostname of called for example acme.com
// *id - an id to be added to end
//
// The format is https://md.pn.id.webshield.io/encrypt_key/<hostnname reversed>#id
//
// FIXME FIXME is this used as now reference source is a resource
//
model.ids.createEncryptKeyId = function createEncryptKeyId(hostname, id) {
  var type = MD_ID_PREFIX + '/encrypt_key';
  return helperCreateId(type, hostname, id);
};
...
model.ids.createPNDataModelId = function createPNDataModelId(hostname, id) {
  var type = MD_ID_PREFIX + '/pn_data_model';
  return helperCreateId(type, hostname, id);
};
