...
PROPERTY = {
  ...
  body:                     URLS.P + 'body', // holds body of a message inside a wrapper such as JWT
...
TYPE = {
  ...
  ProvisionQuery:         URLS.T + 'ProvisionedQuery',

  //
  CreateProvisionStore:   URLS.T + 'CreateProvisionStore',
  ProvisionStore:         URLS.T + 'ProvisionedStore'
...
model.ids.createRequestId = function createRequestId(hostname, id) {
  ...
  rqId = appendReversedHostname(rqId, hostname);
  rqId = rqId + '#' + id;
  return rqId;
};
...
// *id - an id to be added to end
//
// The format is https://pn.id.websheild.io/provision/<hostname reversed>#id
//
model.ids.createProvisionId = function createProvisionId(hostname, id) {
  assert(hostname, 'PNDataModel.createProvisionId requires a hostname param');
  assert(id, 'PNDataModel.createProvisionId requires an id param');

  var pvId = 'https://pn.id.webshield.io/provisions';
  pvId = appendReversedHostname(pvId, hostname);
  pvId = pvId + '#' + id;
  return pvId;
};
