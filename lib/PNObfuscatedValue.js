...
2.	The actual obfuscated value produced by the obfuscation service, for example the output AWS Encryption, Ionic, token, etc
...
//
// Return true if the value is an obfuscated value
//
utils.isOV = function isOv(ov) {
  'use strict';
  if (!ov['@value'] && !ov['@type']) { // an ov needs both these
    return false;
  }

  // if the type contains an privacy action id then assume a pai
  if (ov['@type'].indexOf('paction_instance') !== -1) {
    return true;
  } else {
    return false;
  }
